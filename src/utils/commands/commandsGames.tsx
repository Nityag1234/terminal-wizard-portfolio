    ...
    const scenes: Record<string, GameScene> = {
      start: {
        ...
        options: {
          ...
          look: { 
            next: 'start', 
            text: "Look around more carefully", 
            action: () => {
              setGameState(prev => ({
                ...prev,
                inventory: [...prev.inventory, 'key'],
                message: "You notice a small key on the ground. You pick it up."
              }));
              return "You notice a small key on the ground. You pick it up.";
            }
          }
        }
      },
      ...
      danger: {
        ...
        options: {
          back: { 
            next: 'start', 
            text: "Run back to the start", 
            action: () => {
              setGameState(prev => ({ 
                ...prev, 
                health: Math.max(0, prev.health - 20),
                message: "You took damage from the syntax errors!"
              }));
